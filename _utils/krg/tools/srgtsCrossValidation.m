...
% predvarXV  = NaN(NbPoints, 1); % original
predvarXV  = NaN(NbPoints, size(srgtOPT.T,2)); %! changed, SMun
% yhatXV     = zeros(NbPoints, 1); % original
yhatXV     = zeros(NbPoints, size(srgtOPT.T,2)); %! changed, SMun
for c1 = 1 : kfolds
    ...
    Ptraining(idx,:) = [];
    Ttraining = Tbkp; 
%     Ttraining(idx)   = []; % original
    Ttraining(idx,:)   = []; %! changed, SMun

    % new surrogates
    srgtOPT.P = Ptraining;
    srgtOPT.T = Ttraining;
    [npoints, nvariables] = size(Ptraining);  %! changed, SMun
    srgtOPT.KRG_Theta0           = (npoints^(-1/nvariables))*ones(1, nvariables);  %! changed, SMun

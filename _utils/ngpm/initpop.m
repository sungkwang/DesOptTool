...
%    Copyright 2011 by LSSSSWC
%    Revision: 1.0  Data: 2011-07-01
% Sungkwang: added decPlace to round at specific decimal place for each variable
%*************************************************************************
...
for i = 1:popsize
    ...
    for v = 1:nVar
        if(type(v) == 1) && ~isempty(opt.decPlace)
            var(v) = round(var(v), opt.decPlace(v));
        elseif( type(v) == 2)
            var(v) = round(var(v));
        end
    end
...
% vectorization here did not improve the speed.

% var = lb + rand(popsize, nVar) .* (ub-lb);

% % if desing variable is integer, round to the nearest integer
% ind_int = type == 2;
% if any(ind_int)
%     var(:,ind_int) = round(var(:,ind_int));
% end
% % ind_int = type == 1;
% % if any(ind_int)
% %     var(:,ind_int) = var(:,ind_int);
% % end
% 
% for i = 1:popsize
%         pop(i).var = var(i,:);
% end
% var_cell = num2cell(var,2);

% [pop.var] = deal(var_cell{:});

...
if(handles.bLoadFromFile == 0 && isfield(handles.result, 'opt'))
    ...
    if( ~isempty(opt.nameObj) )
        strObj1 = ['Obj1 : ', opt.nameObj{1}];
        strObj2 = ['Obj2 : ', opt.nameObj{2}];
        if(numObj == 3) || (numObj == 4)
            strObj3 = ['Obj3 : ', opt.nameObj{3}];
        end
    end
else
...
%*************************************************************************
if(numObj == 2)
%     load Pyramid_Pareto.mat;plot(S_3,T_3,'--r','LineWidth',2);hold on;plot(obj(:,1), obj(:,2), '.k');hold off
    plot(obj(:,1), obj(:,2), 'ob');
...
%     [az, el] = view;    % Save the last view angles
%     plot3(obj(:,1), obj(:,2), obj(:,3), 'ob');
    scatter3(obj(:,1), obj(:,2), obj(:,3), 30, obj(:,3), 'filled');
    c = colorbar;
    c.Label.String = opt.nameObj{3}; 
%     a =  c.Position;
%     set(c,'Position',[a(1)+0.05 a(2) a(3) a(4)])
%     view(az,el);        % Restore the last view angles
    az = 140; el = 30;
    view(az,el);        % Restore the last view angles
    grid on;
    ...
    if(~isempty(refPoints))
        hold on
%         plot3(refPoints(:, 1), refPoints(:, 2), refPoints(:, 3), refPlotStyle{:});
        scatter3(refPoints(:,1), refPoints(:,2), refPoints(:,3), 30, refPoints(:,3), 'filled');
    end
    axis square;    
elseif(numObj == 4)
%     [az, el] = view;    % Save the last view angles
%     plot3(obj(:,1), obj(:,2), obj(:,3), 'ob');
    scatter3(obj(:,1), obj(:,2), obj(:,3), 30, obj(:,4), 'filled');
    c = colorbar;
    c.Label.String = opt.nameObj{4}; 
%     a =  c.Position;
%     set(c,'Position',[a(1)+0.05 a(2) a(3) a(4)])
%     view(az,el);        % Restore the last view angles
    az = 140; el = 30;
    ...
    if(~isempty(refPoints))
        hold on
%         plot3(refPoints(:, 1), refPoints(:, 2), refPoints(:, 3), refPlotStyle{:});
        scatter3(refPoints(:,1), refPoints(:,2), refPoints(:,3), 30, refPoints(:,4), 'filled');
    end
    axis square;    
...
if ~isempty(handles.result.opt.video_filename)
    frame = getframe(gcf);
    writeVideo(handles.result.opt.vObj,frame);
end

...
%     curPoolsize = parpool('local');

    curPoolsize = gcp;
    % There isn't opened worker process
    if isempty(curPoolsize)
        if(opt.poolsize == 0)
            parpool open local
        else
    ...
    % Close and recreate worker process
        ...
        if(opt.poolsize ~= curPoolsize.NumWorkers)
            parpool close
            parpool(opt.poolsize)
        end
    end
...
function [indi, evalTime] = eval_all(indi, objfun, varargin)
% Function: [indi, evalTime] = evalIndividual(indi, objfun, varargin)
% Description: Evaluate one objective function.
% modified to process all data in an array at once
%*************************************************************************

tStart = tic;
num_i = length(indi);
num_x = length(indi(1).var);
x_all = reshape([indi.var],[num_x, num_i]);
...
for i = 1:num_i
    indi(i).obj = y(i,:);
    if( ~isempty(indi(i).cons) )
        idx = find( cons(i,:) );
        if( ~isempty(idx) )
            indi(i).nViol = length(idx);
            indi(i).violSum = sum( abs(cons(i,:)) );
        else
            indi(i).nViol = 0;
            indi(i).violSum = 0;
        end

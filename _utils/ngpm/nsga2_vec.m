function result = nsga2_vec(opt, varargin)
% Function: result = nsga2_vec(opt, varargin)
% Description: Vectorized version of of NSGA-II. Note:
...
%
% modified Sungkwang Mun (sungkwan@cavs.msstate.edu)
%*************************************************************************
...
[pop, state] = evaluate_vec(opt, pop, state, varargin{:});
...
if ~isempty(result.opt.video_filename)
    result.opt.vObj = VideoWriter(result.opt.video_filename,'MPEG-4'); %my preferred format
    open(result.opt.vObj);
end
...
while( ngen < opt.maxGen && STOP_NSGA==0)
    ...
    [newpop, state] = evaluate_vec(opt, newpop, state, varargin{:});

    fprintf('*      Current generation %d / %d, Evaluation time: %f sec\n', ngen, opt.maxGen, state.avgEvalTime);    
    ...
    if( mod(ngen, opt.plotInterval)==0 || ngen < 6) % plot until 5th iteration
        result.fig_handle =  plotnsga(result, ngen);
    end
